...
import { type Point, type PolygonFiller } from './interfaces/PolygonFiller';
...
export class PolygonFillingPresenter {
  ...
  constructor(
    ...
    @inject(TYPES.POLYGON_FILLER) private readonly filler: PolygonFiller
  ) {
    this.figure = [];
    ...
    this.screen.addEventListener('mousedown', async (e) => {
      ...
      if (
        ...
        await this.filler.fillPolygon(this.figure, '#51b52c');
        if (drawEdges) {
